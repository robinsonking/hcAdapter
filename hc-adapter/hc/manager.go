...
type WalletManager struct {
	...
	Blockscanner    *HCBlockScanner               //区块扫描器
...
func NewWalletManager() *WalletManager {
	...
	//区块扫描器
	wm.Blockscanner = NewHCBlockScanner(&wm)
...
func (wm *WalletManager) EncryptWallet(password string) error {
	...
	_, err := wm.WalletClient.Call("walletlock", request)
	if err != nil {
		return err
	}
	return nil
...
//DumpWallet 导出钱包所有私钥文件   //hc有区别
func (wm *WalletManager) DumpPrivate(filename string) error {

	request := []interface{}{
		filename,
	}

	_, err := wm.WalletClient.Call("dumpprivkey", request)
	if err != nil {
		return err
	}
...
func (wm *WalletManager) SignRawTransactionInCoreWallet(txHex, walletID string, key *hdkeystore.HDKey, utxos []*Unspent) (string, error) {
	...
	for _, u := range utxos {
		...
		privateKey, _ := secp256k1.PrivKeyFromBytes(secp256k1.S256(), keyBytes)
		...
		wif, err := hcutil.NewWIF(privateKey, &cfg, suites[0])
		if err != nil {
			return "", err
		}
