...
/////////////////////////////////////////////////////////////////////////////////////SM2 STANDARD CURVE///////////////////////////////////////////////////////////////////////////////////////////////////////
static const uint8_ow curve_sm2_std_p[32]   = {0xFF,0xFF,0xFF,0xFE,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x00,0x00,0x00,0x00,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF};
static const uint8_ow curve_sm2_std_a[32]   = {0xFF,0xFF,0xFF,0xFE,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x00,0x00,0x00,0x00,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFC};
static const uint8_ow curve_sm2_std_b[32]   = {0x28,0xE9,0xFA,0x9E,0x9D,0x9F,0x5E,0x34,0x4D,0x5A,0x9E,0x4B,0xCF,0x65,0x09,0xA7,0xF3,0x97,0x89,0xF5,0x15,0xAB,0x8F,0x92,0xDD,0xBC,0xBD,0x41,0x4D,0x94,0x0E,0x93};
static const uint8_ow curve_sm2_std_x[32]   = {0x32,0xC4,0xAE,0x2C,0x1F,0x19,0x81,0x19,0x5F,0x99,0x04,0x46,0x6A,0x39,0xC9,0x94,0x8F,0xE3,0x0B,0xBF,0xF2,0x66,0x0B,0xE1,0x71,0x5A,0x45,0x89,0x33,0x4C,0x74,0xC7};
static const uint8_ow curve_sm2_std_y[32]   = {0xBC,0x37,0x36,0xA2,0xF4,0xF6,0x77,0x9C,0x59,0xBD,0xCE,0xE3,0x6B,0x69,0x21,0x53,0xD0,0xA9,0x87,0x7C,0xC6,0x2A,0x47,0x40,0x02,0xDF,0x32,0xE5,0x21,0x39,0xF0,0xA0};
static const uint8_ow curve_sm2_std_n[32]   = {0xFF,0xFF,0xFF,0xFE,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x72,0x03,0xDF,0x6B,0x21,0xC6,0x05,0x2B,0x53,0xBB,0xF4,0x09,0x39,0xD5,0x41,0x23};
...
uint16_ow sm2_std_genPubkey(uint8_ow *prikey, uint8_ow *pubkey)
{
    ...
    curveParam -> p = (uint8_ow *)curve_sm2_std_p;
    curveParam -> a = (uint8_ow *)curve_sm2_std_a;
    curveParam -> b = (uint8_ow *)curve_sm2_std_b;
    curveParam -> x = (uint8_ow *)curve_sm2_std_x;
    curveParam -> y = (uint8_ow *)curve_sm2_std_y;
    curveParam -> n = (uint8_ow *)curve_sm2_std_n;
...
void sm2_get_e(ECC_CURVE_PARAM *curveParam, uint8_ow *ID, uint16_ow IDlen, uint8_ow *key, uint8_ow *message, uint16_ow message_len, uint8_ow mode, uint8_ow *e)
{
    uint8_ow ENTLA[2] = {0};
    uint8_ow *pubkey = NULL;
    ...
    {
        pubkey = calloc(ECC_LEN * 2, sizeof(uint8_ow));
...
uint16_ow sm2_std_sign(uint8_ow *prikey, uint8_ow *ID, uint16_ow IDlen, uint8_ow *message, uint16_ow message_len,uint8_ow *rand,uint8_ow hash_flag, uint8_ow *sig)
{
    ECC_CURVE_PARAM *curveParam = NULL;
    ECC_POINT *point = NULL;
    uint8_ow *e = NULL, *k = NULL, *tmp = NULL;
...
static void sm2Kdf(uint8_ow *Z, uint16_ow Zlen, int klen_bit, uint8_ow *key)
{
#define V 256
    uint32_ow hlen1, i;
    uint8_ow generater[4] = {0};
    SM3_CTX *sm3_ctx = NULL;
    uint8_ow *tmp = NULL;
    ...
    sm3_ctx = calloc(1, sizeof(SM3_CTX));
    tmp = calloc(SM3_DIGEST_LENGTH, sizeof(uint8_ow));
...
uint16_ow sm2_std_enc(uint8_ow *pubkey, uint8_ow *plain, uint16_ow plain_len, uint8_ow *cipher, uint16_ow *cipher_len)
{
    ...
    *cipher = 0x04;
    ...
    tmp = calloc(ECC_LEN * 2, sizeof(uint8_ow));
    
    while(1)
    {
        k = calloc(ECC_LEN, sizeof(uint8_ow));
        bigrand_get_rand_range(k, curveParam -> n, pubkey, ECC_LEN * 2, plain, plain_len);
...
void sm2_ka_KDF(uint8_ow *x, uint8_ow *y, uint8_ow *Zinitiator, uint8_ow *Zresponder, uint16_ow klen_bit, uint8_ow*key)
{
#define V 256
    uint32_ow hlen1, i;
    uint8_ow generater[4] = {0};
...
void sm2_ka_check(uint8_ow value, uint8_ow *Zinitiator, uint8_ow *Zresponder, uint8_ow *Rinitiator, uint8_ow *Rresponder, ECC_POINT *UV, uint8_ow *S)
{
    SM3_CTX *sm3_ctx = NULL;
    sm3_ctx = calloc(1, sizeof(SM3_CTX));
...
{
    bigrand_get_rand_range(tmpPriInitiator, (uint8_ow *)curve_sm2_std_n, 0, 0, 0, 0);
                                    ...
                                    uint8_ow *key)
{
    ...
    *(tmp2 + ECC_LEN / 2) |= 0x80;
    ...
    Zinitiator = calloc(SM3_DIGEST_LENGTH, sizeof(uint8_ow));
    Zresponder = calloc(SM3_DIGEST_LENGTH, sizeof(uint8_ow));
