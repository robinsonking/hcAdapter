 ...
 */
static void padkey(uint8_ow *K,uint8_ow *pad,uint16_ow len,uint8_ow *out)
{
    uint16_ow i;
    if(!len)
        return;
 ...
 */
void HMAC(uint8_ow *K,uint16_ow Klen,uint8_ow *M,uint16_ow Mlen,uint8_ow *out,uint32_ow Hmac_Hash_Alg)
{
    uint16_ow blockByte;
    uint8_ow *KCopy=NULL,*ipad=NULL,*opad=NULL,*temp_result=NULL,*keypad=NULL;
    ...
    KCopy=calloc(blockByte,sizeof(uint8_ow));
    ipad = calloc(blockByte,sizeof(uint8_ow));
    opad = calloc(blockByte,sizeof(uint8_ow));
    keypad = calloc(blockByte,sizeof(uint8_ow));
    ...
    switch(Hmac_Hash_Alg)
    {
        case HMAC_SHA256_ALG:
            ...
            temp_result = calloc(32,sizeof(uint8_ow));
            if(Klen > blockByte)
            {
        ...
        case HMAC_SHA512_ALG:
            ...
            temp_result = calloc(64,sizeof(uint8_ow));
            if(Klen > blockByte)
            {
