...
(((uint32_ow) *((strptr)+3) << 24) |  \
((uint32_ow) *((strptr)+2) << 16) |   \
((uint32_ow) *((strptr)+1) <<  8) |   \
((uint32_ow) *(strptr)))              \
...
void ripemd160_init(RIPEMD160_CTX *ctx)
{
    if(!ctx)
        return;
    memset((uint8_ow *)ctx, 0, sizeof(RIPEMD160_CTX));
...
{
    uint32_ow aa = ctx->state[0], bb = ctx->state[1], cc = ctx->state[2],
    dd = ctx->state[3], ee = ctx->state[4];
    uint32_ow aaa = ctx->state[0], bbb = ctx->state[1], ccc = ctx->state[2],
...
/*
 ...
 @notoce:none
 */
void ripemd160_update(RIPEMD160_CTX *ctx,uint8_ow *msg,uint32_ow msg_len)
{
    uint32_ow fill,left;
    if(!msg_len)
        return;
    ...
    {
        memcpy((uint8_ow *)(ctx -> buffer) + left, msg, fill);
         compress(ctx);
        msg += fill;
        msg_len  -= fill;
        left = 0;
    ...
    if(msg_len > 0)
        memcpy((uint8_ow *)(ctx -> buffer) + left, msg, msg_len);
    return;
}
/*
 ...
 @notice:nothing
 */
void ripemd160_final(RIPEMD160_CTX *ctx,uint8_ow digest[20])
{
        uint32_ow last, padn;
        uint32_ow high, low;
        uint8_ow msglen[8];
        ...
        ripemd160_update(ctx, (uint8_ow *)ripemd160_padding, padn);
...
/*
 ...
 @notice:none
 */
void ripemd160_hash(uint8_ow *msg,uint32_ow msg_len,uint8_ow digest[20])
{
    RIPEMD160_CTX ctx;
    if(! msg_len)
        return;
