...
static void _MD5Transform(uint32_ow[4], const uint8_ow[64]);
//static void _Encode(unsigned char *, unsigned int *, unsigned int);
static void _Encode(uint8_ow *output, uint32_ow *input, uint32_ow len);
static void _Decode(uint32_ow *output, const uint8_ow *input, uint32_ow len);
...
/*
 ...
 @notoce:none
 */
void md5_update(MD5_CTX * ctx, const uint8_ow *msg, const uint32_ow msg_len)
{
    ...
    if ((ctx->count[0] += ((uint32_ow) msg_len << 3)) < ((uint32_ow) msg_len << 3))
        ctx->count[1]++;
    ctx->count[1] += ((uint32_ow) msg_len >> 29);
...
/*
 ...
 @notice:nothing
 */
void md5_final(MD5_CTX * ctx,uint8_ow digest[16])
{
    uint8_ow bits[8];
    uint32_ow index, padLen;
    if(!ctx || !digest)
        return;
    /* Save number of bits */
...
static void _MD5Transform(uint32_ow state[4], const uint8_ow block[64])
{
    uint32_ow a = state[0],
    b = state[1],
    c = state[2],
...
static void _Encode(uint8_ow *output,  uint32_ow *input, uint32_ow len)
{
    ...
    for (i = 0, j = 0; j < len; i++, j += 4) {
        output[j] = (uint8_ow) (input[i] & 0xff);
        output[j + 1] = (uint8_ow ) ((input[i] >> 8) & 0xff);
        output[j + 2] = (uint8_ow ) ((input[i] >> 16) & 0xff);
        output[j + 3] = (uint8_ow ) ((input[i] >> 24) & 0xff);
    }
}
...
static void _Decode(uint32_ow *output, const uint8_ow *input, uint32_ow len)
{
   uint32_ow i, j;
    ...
    if(!len)
        return;
    for (i = 0, j = 0; j < len; i++, j += 4) {
        output[i] = ((uint32_ow) input[j]) | (((uint32_ow) input[j + 1]) << 8) |
        (((uint32_ow ) input[j + 2]) << 16) | (((uint32_ow) input[j + 3]) << 24);
    }
    return;
}
...
/*
 ...
 @notice:none
 */
void  md5_hash(const uint8_ow *msg,uint32_ow msg_len,uint8_ow digest[16])
{
     MD5_CTX ctx;
    if(!msg || !digest)
        return;
