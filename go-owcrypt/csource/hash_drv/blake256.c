...
static const uint8_ow blake256_sigma[][16] =
{
...
static const uint32_ow blake256_u256[16] =
{
...
static const uint8_ow blake256_padding[129] =
{
...
static void blake256_compress( BLAKE256_CTX *ctx, const uint8_ow *block )
{
uint32_ow v[16], m[16], i;
#define BLAKE256_ROT(x,n) (((x)<<(32-n))|( (x)>>(n)))
#define BLAKE256_G(a,b,c,d,e)          \
v[a] += (m[blake256_sigma[i][e]] ^ blake256_u256[blake256_sigma[i][e+1]]) + v[b]; \
v[d] = BLAKE256_ROT( v[d] ^ v[a],16);        \
v[c] += v[d];           \
v[b] = BLAKE256_ROT( v[b] ^ v[c],12);        \
v[a] += (m[blake256_sigma[i][e+1]] ^ blake256_u256[blake256_sigma[i][e]])+v[b]; \
...
v[b] = BLAKE256_ROT( v[b] ^ v[c], 7);
    ...
    v[ 8] = ctx->s[0] ^ blake256_u256[0];
    v[ 9] = ctx->s[1] ^ blake256_u256[1];
    v[10] = ctx->s[2] ^ blake256_u256[2];
    v[11] = ctx->s[3] ^ blake256_u256[3];
    ...
    for( i = 0; i < 14; ++i )
    {
        ...
        BLAKE256_G( 1,  5,  9, 13,  2 );
        BLAKE256_G( 2,  6, 10, 14,  4 );
        BLAKE256_G( 3,  7, 11, 15,  6 );
        /* diagonal step */
        BLAKE256_G( 0,  5, 10, 15,  8 );
        BLAKE256_G( 1,  6, 11, 12, 10 );
        BLAKE256_G( 2,  7,  8, 13, 12 );
        BLAKE256_G( 3,  4,  9, 14, 14 );
    }
    for( i = 0; i < 16; ++i )
        ctx->h[i % 8] ^= v[i];
    for( i = 0; i < 8 ; ++i )
...
void blake256_final( BLAKE256_CTX *ctx, uint8_ow *digest )
{
    uint8_ow msglen[8], zo = 0x01, oo = 0x81;
    uint32_ow lo = ctx->t[0] + ( ctx->buflen << 3 ), hi = ctx->t[1];
