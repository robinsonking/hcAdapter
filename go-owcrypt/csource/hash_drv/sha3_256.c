...
static uint64_ow sha3_256_round_constants[sha3_256_NumberOfRounds] =
{
...
/* sha3-256 theta() transformation */
static void sha3_256_keccak_theta(uint64_ow *A)
{
    uint64_ow D[5];
...
/* sha3-256 pi() transformation */
static void sha3_256_keccak_pi(uint64_ow *A)
{
    uint64_ow A1;
    A1 = A[1];
    A[ 1] = A[ 6];
...
static void sha3_256_keccak_chi(uint64_ow *A)
{
    uint64_ow A0, A1;
    SHA3_256CHI_STEP(0);
    SHA3_256CHI_STEP(5);
...
static void sha3_256_permutation(uint64_ow *state)
{
    int round;
 ...
 */
static void sha3_256_process_block(uint64_ow hash[25], const uint64_ow *block, size_t block_size)
{
    /* expanded loop */
 ...
 */
void sha3_2556_update(SHA3_256_CTX *ctx, const uint8_ow *msg, uint32_ow msglen)
{
    uint32_ow index = (uint32_ow)ctx->rest;
    uint32_ow block_size = (uint32_ow)ctx->block_size;
    if (ctx->rest & SHA3_256_FINALIZED) /* too late for additional input */
        return;
    ctx->rest = (uint32_ow)((ctx->rest + msglen) % block_size);
    /* fill partial block */
    if (index)
    {
        int left = block_size - index;
        memcpy((uint8_ow*)ctx->message + index, msg, (msglen < left ? msglen : left));
        if (msglen < left)
            return;
 ...
 */
void sha3_256_final(SHA3_256_CTX *ctx, uint8_ow* digest)
{
...
*/
void sha3_256_hash(const uint8_ow *msg,uint32_ow msg_len,uint8_ow *digest)
{
    SHA3_256_CTX ctx;
    sha3_256_init(&ctx);
