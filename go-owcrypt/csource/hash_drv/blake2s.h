...
{
    uint8_ow  digest_length; //digest byte length, an integer in [1,32]
    uint8_ow  key_length;    //key byte length, an integer in [0,32]
    ...
    uint8_ow  node_offset[6];// 6 bytes,range in[1,2^48-1](set to 0 for the first, leftmost, leaf, or in sequential mode)
    uint8_ow  node_depth;    //1 byte,an integer in [1, 255] (set to 255 if unlimited, and to 1 only in sequential mode)
    uint8_ow  inner_length;  //1 byte,range in [0,32](set to 0 in sequential mode)
    uint8_ow  salt[BLAKE2S_SALTBYTES]; //8 bytes,(set to all-NULL by default)
    uint8_ow  personal[BLAKE2S_PERSONALBYTES];  //8 bytes,(set to all-NULL by default)
};
struct blake2s_ctx_st
{
    uint32_ow h[8]; //store hash intermidiate state
    uint32_ow t[2];//counter
    uint32_ow f[2];//finalization flags
    uint8_ow  buf[BLAKE2S_BLOCKBYTES];//store block message to deal with
 ...
 */
void blake2s_init(BLAKE2S_CTX *ctx,uint8_ow *key,uint16_ow key_len,uint16_ow digest_len);
/*
 ...
 */
void blake2s_update(BLAKE2S_CTX *ctx, const uint8_ow *msg, uint32_ow mas_len);
 ...
 */
void blake2s_final(BLAKE2S_CTX *ctx,uint8_ow *digest,uint32_ow digest_len);
 ...
 */
void blake2s(uint8_ow *msg, uint16_ow msg_len,uint8_ow *key,uint16_ow key_length, uint8_ow digest_len, uint8_ow *digest);
