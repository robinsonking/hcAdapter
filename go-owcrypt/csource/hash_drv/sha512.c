...
static const uint64_ow K_SHA512[80] =
{
...
{
    memset((uint8_ow*)ctx, 0, sizeof(SHA512_CTX));
...
void sha512_process(SHA512_CTX *ctx, uint8_ow data[128])
{
    int i;
    uint64_ow temp1, temp2, W[80];
    uint64_ow A, B, C, D, E, F, G, H;
...
/*
 ...
 @notoce:none
 */
void sha512_update(SHA512_CTX *ctx, uint8_ow *msg, uint16_ow msg_len)
{
    size_t fill;
    unsigned int left;
    ...
    ctx -> total[0] += (uint64_ow)msg_len;
    
    if(ctx -> total[0] < (uint64_ow)msg_len)
        ctx -> total[1]++;
...
/*
 ...
 @notice:nothing
 */
void sha512_final(SHA512_CTX *ctx, uint8_ow *digest)
{
    size_t last, padn;
    uint64_ow high, low;
    ...
    sha512_update(ctx, (uint8_ow *)sha512_padding, padn);
...
/*
 ...
 @notice:nothing
 */
void sha512_hash(uint8_ow *msg, uint32_ow msg_len, uint8_ow *digest)
{
    SHA512_CTX *ctx = NULL;
