...
static void md4_transform (uint32_ow [4], const uint8_ow [64]);
static void Encode (uint8_ow *,  uint32_ow *, uint32_ow);
static void Decode (uint32_ow *, const uint8_ow *, uint32_ow);
...
#define GG_MD4(a, b, c, d, x, s) {(a) += G_MD4 ((b), (c), (d)) + (x) + (uint32_ow)0x5a827999; (a) = ROTATE_LEFT_MD4 ((a), (s));}

#define HH_MD4(a, b, c, d, x, s) {(a) += H_MD4 ((b), (c), (d)) + (x) + (uint32_ow)0x6ed9eba1; (a) = ROTATE_LEFT_MD4 ((a), (s));}
...
/*
 ...
 @notoce:none
 */
void md4_update (MD4_CTX *ctx, const uint8_ow *msg, uint32_ow msg_len)
{
    uint32_ow i, index, partLen;
    
    /* Compute number of bytes mod 64 */
    index = (uint32_ow)((ctx->count[0] >> 3) & 0x3F);
    
    /* Update number of bits */
    if ((ctx->count[0] += ((uint32_ow)msg_len << 3))< ((uint32_ow)msg_len << 3))
        ctx->count[1]++;
...
/*
 ...
 @notice:nothing
 */
void md4_final (MD4_CTX *ctx,uint8_ow digest[16])
{
...
static void md4_transform (uint32_ow state[4], const uint8_ow block[64])
{
    uint32_ow a = state[0], b = state[1], c = state[2], d = state[3], x[16];
...
static void Encode (unsigned char *output, uint32_ow *input, unsigned int len)
{
    unsigned int i, j;
...
static void Decode (uint32_ow *output, const uint8_ow *input, uint32_ow len)
{
    unsigned int i, j;
    for (i = 0, j = 0; j < len; i++, j += 4)
        output[i] = ((uint32_ow)input[j]) | (((uint32_ow)input[j+1]) << 8) | (((uint32_ow)input[j+2]) << 16) | (((uint32_ow)input[j+3]) << 24);
}
...
/*
 ...
 @notice:none
 */
void  md4_hash(const uint8_ow *msg,uint32_ow msg_len,uint8_ow digest[16])
{
    MD4_CTX ctx;
    if(!msg || !digest)
        return;
