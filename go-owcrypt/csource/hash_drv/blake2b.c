...
static void blake2b_compress(BLAKE2B_CTX *S,const uint8_ow *blocks,size_t len);

static const uint64_ow blake2b_IV[8] =
{
    0x6a09e667f3bcc908U,
    0xbb67ae8584caa73bU,
    0x3c6ef372fe94f82bU,
...
static const uint8_ow blake2b_sigma[12][16] =
{
...
static uint64_ow rotr64(const uint64_ow w, const unsigned int c)
{
    return (w >> c) | (w << (64 - c));
}
...
static void blake2b_init_param(BLAKE2B_CTX *S, const BLAKE2B_PARAM *P)
{
    ...
    for (i = 0; i < 8; ++i) {
        memcpy((uint8_ow *)&tmp, (uint8_ow *)(p + sizeof(S->h[i]) * i), 8);
        S->h[i] ^= tmp;
    }
}
...
static void blake2b_compress(BLAKE2B_CTX *S,const uint8_ow *blocks,size_t len)
{
    ...
    do {
        for (i = 0; i < 16; ++i)
        {
            memcpy((uint8_ow *)&m[i], (uint8_ow *)(blocks + i * sizeof(m[i])), 8);
        }
        S->t[0] += increment;
...
void blake2b_update(BLAKE2B_CTX *ctx, uint8_ow *msg, uint32_ow msg_len)
{
    ...
    if (msg_len > fill)
    {
        ...
        {
            uint32_ow stashlen = msg_len % BLAKE2B_BLOCKBYTES;
...
void blake2b_final(BLAKE2B_CTX *ctx,uint8_ow *msg, uint8_ow msg_len)
{
    ...
    memcpy(msg, (uint8_ow *)&ctx->h[0], msg_len);
    //return 1;
}
