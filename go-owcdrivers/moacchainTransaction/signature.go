...
func moac_hmac_rfc6979_init(key []byte, keylen int) ([]byte, int) {
	k := make([]byte, 32)
	v := make([]byte, 32)
	out := make([]byte, 64)
...
*/
func moac_hmac_rfc6979_gnerate(k, v []byte, retry, nouncelen int) ([]byte, int) {
...
func moac_nonce_function_rfc6979(msg, key, algo, extradata []byte, counter uint32) []byte {
	...
	ret, retry := moac_hmac_rfc6979_init(keydata, keylen)

	for i := uint32(0); i <= counter; i++ {
		ReTry := retry
		temp, retry := moac_hmac_rfc6979_gnerate(ret[:32], ret[32:], ReTry, 32)
...
func moac_set_uint64(a []byte) []uint64 {
...
func moac_set_uint32(a []byte) []uint32 {
...
func moac_check_overflow_uint64(a []byte) bool {
	var yes bool
	var no bool
	yes = false
	no = false
	curveOrder := owcrypt.GetCurveOrder(owcrypt.ECC_CURVE_SECP256K1)
	a_uint64 := moac_set_uint64(a)
	curveOrder_uint64 := moac_set_uint64(curveOrder)
...
func moac_check_overflow_uint32(a []byte) bool {
	var yes bool
	var no bool
	yes = false
	no = false
	curveOrder := owcrypt.GetCurveOrder(owcrypt.ECC_CURVE_SECP256K1)
	a_uint32 := moac_set_uint32(a)
	curveOrder_uint32 := moac_set_uint32(curveOrder)
...
func moac_check_is_zero(a []byte) bool {
	b := make([]uint64, len(a)>>3)
	b = moac_set_uint64(a)
	return ((b[3] == 0) && (b[2] == 0) && (b[1] == 0) && (b[0] == 0))
}

func moac_signatureInner(prikey []byte, hash []byte, nounce []byte) ([]byte, uint16) {
	var recid byte
...
func MoacSignature(prikey []byte, hash []byte) ([]byte, uint16) {
	...
	prikey_overflow := moac_check_overflow_uint64(prikey)
	prikey_IsZero := moac_check_is_zero(prikey)
	if !prikey_overflow && !prikey_IsZero {
		for {
			nounce := moac_nonce_function_rfc6979(hash, prikey, nil, nil, counter)
			nounce_overflow := moac_check_overflow_uint32(nounce)
			nounce_IsZero := moac_check_is_zero(nounce)
			if !nounce_overflow && !nounce_IsZero {
				signature, ret = moac_signatureInner(prikey, hash, nounce)
				if ret == owcrypt.SUCCESS {
					break
				}
			}
