...
func DecodeCheck(address string) ([]byte, []byte, error) {
	...
	hashBlack := owcrypt.Hash(ret[:len(ret)-4], 0, owcrypt.HASH_ALG_BLAKE256)
	checksum := owcrypt.Hash(hashBlack[:], 0, owcrypt.HASH_ALG_BLAKE256)[:4]
	for i := 0; i < 4; i++ {
...
func EncodeCheck(prefix []byte, hash []byte) string {
	...
	checksum := owcrypt.Hash(data, 0, owcrypt.HASH_ALG_BLAKE256)[:4]
