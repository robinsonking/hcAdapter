...
func DecodeCheck(address string) ([]byte, error) {
	...
	checksum := owcrypt.Hash(owcrypt.Hash(ret[:len(ret)-4], 32, owcrypt.HASH_ALG_BLAKE2B), 0, owcrypt.HASH_ALG_KECCAK256)[:4]
	for i := 0; i < 4; i++ {
...
func EncodeCheck(prefix byte, hash []byte) string {
	...
	checksum := owcrypt.Hash(owcrypt.Hash(data, 32, owcrypt.HASH_ALG_BLAKE2B), 0, owcrypt.HASH_ALG_KECCAK256)[:4]
