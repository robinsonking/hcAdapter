...
func (decoder *TransactionDecoder) CreateRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	if rawTx.Coin.IsContract {
		...
		return decoder.CreateHCRawTransaction(wrapper, rawTx)
	}
}
...
func (decoder *TransactionDecoder) SignRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	if rawTx.Coin.IsContract {
		...
		return decoder.SignHCRawTransaction(wrapper, rawTx)
	}
}
...
func (decoder *TransactionDecoder) VerifyRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	if rawTx.Coin.IsContract {
		...
		return decoder.VerifyHCRawTransaction(wrapper, rawTx)
	}
}
...
func (decoder *TransactionDecoder) CreateSummaryRawTransaction(wrapper openwallet.WalletDAI, sumRawTx *openwallet.SummaryRawTransaction) ([]*openwallet.RawTransaction, error) {
	...
	if sumRawTx.Coin.IsContract {
		...
		rawTxWithErrArray, err = decoder.CreateHCSummaryRawTransaction(wrapper, sumRawTx)
	}
	if err != nil {
		return nil, err
	}
...
//CreateRawTransaction 创建交易单
func (decoder *TransactionDecoder) CreateHCRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	err = decoder.createHCRawTransaction(wrapper, rawTx, usedUTXO, outputAddrs)
	if err != nil {
		return err
	}
...
//SignRawTransaction 签名交易单
func (decoder *TransactionDecoder) SignHCRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
...
//VerifyRawTransaction 验证交易单，验证交易单并返回加入签名后的交易单
func (decoder *TransactionDecoder) VerifyHCRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
...
//CreateHCSummaryRawTransaction 创建HC汇总交易
func (decoder *TransactionDecoder) CreateHCSummaryRawTransaction(wrapper openwallet.WalletDAI, sumRawTx *openwallet.SummaryRawTransaction) ([]*openwallet.RawTransactionWithError, error) {
		...
		if i == len(sumAddresses)-1 || len(sumUnspents) >= decoder.wm.Config.MaxTxInputs {
			...
			*/
			...
			if sumAmount.GreaterThan(decimal.Zero) {
				...
				createErr = decoder.createHCRawTransaction(wrapper, rawTx, sumUnspents, outputAddrs)
...
//createHCRawTransaction 创建HC原始交易单
func (decoder *TransactionDecoder) createHCRawTransaction(
...
func (decoder *TransactionDecoder) CreateSummaryRawTransactionWithError(wrapper openwallet.WalletDAI, sumRawTx *openwallet.SummaryRawTransaction) ([]*openwallet.RawTransactionWithError, error) {
	if sumRawTx.Coin.IsContract {
		...
		return decoder.CreateHCSummaryRawTransaction(wrapper, sumRawTx)
	}
}
