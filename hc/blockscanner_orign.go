...
type HCBlockScanNotificationObject interface {
	...
	//@required
	HCBlockScanNotify(block *Block, txs []*Transaction) error
}
...
func (bs *HCBlockScanner) SetScanBlockTaskOrigin() {
	bs.SetTask(bs.ScanBlockTaskOrigin)
}

//ScanBlockTask 扫描任务
func (bs *HCBlockScanner) ScanBlockTaskOrigin() {
	...
	for {
		...
		if currentHash != block.Previousblockhash && len(currentHash) > 0 {
			...
			if forkBlock != nil {
				...
				//通知分叉区块给观测者，异步处理
				bs.NewHCBlockNotify(forkBlock, isFork)
			}
			...
			//通知新区块给观测者，异步处理
			bs.NewHCBlockNotify(block, isFork)
		}
...
func (bs *HCBlockScanner) ScanTxMemPoolOrigin() {
	...
	//通知内存池交易单给观测者
	bs.NewHCBlockNotify(block, false)
}
...
//AddObserver 添加观测者
func (bs *HCBlockScanner) AddHCBlockObserver(obj HCBlockScanNotificationObject) error {
	bs.Mu.Lock()
	...
	if _, exist := bs.HCBlockObservers[obj]; exist {
		//已存在，不重复订阅
		return nil
	}

	bs.HCBlockObservers[obj] = true

	return nil
}

//RemoveObserver 移除观测者
func (bs *HCBlockScanner) RemoveHCBlockObserver(obj HCBlockScanNotificationObject) error {
	bs.Mu.Lock()
	defer bs.Mu.Unlock()

	delete(bs.HCBlockObservers, obj)

	return nil
}

//newBlockNotify 获得新区块后，通知给观测者
func (bs *HCBlockScanner) NewHCBlockNotify(block *Block, isFork bool) {
	block.Fork = isFork
	for o, _ := range bs.HCBlockObservers {
		o.HCBlockScanNotify(block, block.txDetails)
	}
}


//GetScannedBlockHeader 获取当前扫描的区块头
func (bs *HCBlockScanner) GetScannedBlockHeaderOrigin() (*openwallet.BlockHeader, error) {
